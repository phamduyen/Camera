 ...
 */
public class AddCatAdminAction extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddCatAdminAction() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (submit != null) {
			...
			if(catBO.addCat(nameCat)){
				RequestDispatcher rd = request.getRequestDispatcher("list-cat");
				rd.forward(request, response);
			} else {
				RequestDispatcher rd = request.getRequestDispatcher("admin/addCat.jsp?msg=Có lỗi trong quá trình xử lý ");
				rd.forward(request, response);
			}
